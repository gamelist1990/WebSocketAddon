    ...
    handler.registerCommand('item', {
        ...
        execute: (message, event) => {
            ...
            if (subCommand === 'give') {
                ...
                player.sendMessage('=== 登録済みカスタムアイテム一覧 (ID順) ===');
                if (customItemsMap.size === 0) {
                    player.sendMessage('§e登録されているカスタムアイテムはありません。');
                } else {
                    const sortedItems = Array.from(customItemsMap.entries())
                        .sort(([keyA], [keyB]) => {
                    ...
                    sortedItems.forEach(([id, customItem]) => {
                ...
                player.sendMessage('§c使用方法: item give <itemID|itemName> [amount] [itemLock] [slot] | item info <itemID|itemName> | item list');
            }
        },
    });
}